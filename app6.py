...
def generate_crypto_question_google():
    ...
    prompt = f"""
        Genera una única pregunta de quiz sobre la asignatura Fundamentos básicos de la Teoría de Conjuntos para estudiantes de la Licenciatura en Computación del primer semestre. La pregunta debe venir acompañada con un escenario teórico (máximo 1-2 párrafos) relacionado con la pregunta. 
...
with st.container():
    ...
    st.markdown(f"Responde **{st.session_state.max_questions} preguntas** generadas por IA y pon a prueba tus conocimientos de Linux.") # Desc actualizada
    #st.divider()
...
# --- Footer --- (sin cambios)
st.caption("Aplicación creada por el Prof. Haller Bracho usando la librería Streamlit de Python y la API de Google AI Studio (Gemini).")
